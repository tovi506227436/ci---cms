...
/*自己编写一般的删除*/
function user_del2(url,id,turl){
	layer.confirm('确认要删除吗？',function(index){	
		data={id:id};		
		$.ajax({
			type:'POST',
			...
			success:function(response){
				...
				if(response=='1'){
					//layer.alert('删除成功', {icon: 6},function(){window.location.href=turl});
					layer.msg('删除成功', {
						closeBtn: true,
					    icon: 1,
					    time: 1000 
...
/*自己编写一般的删除*/
function resourceDelect(title,url,id,turl){
...
/*自己编写一般的删除*/
function resourceDelect2(title,url,id,turl){
	layer.confirm(title,{icon:3},function(index){	
		data={id:id};
		$.ajax({
			type:'POST',
...
function delectData(title,url,skipUrl){	
	var arrUserid="";
